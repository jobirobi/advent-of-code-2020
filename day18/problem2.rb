...
# (a different part of the code will handle parentheses)
# dothemaths() takes a string in the form (eg) "1 + 2 * 3 + 4 * 5 ..."
# and returns the value of that expression
def dothemaths(expr)

  # do additions first (eg) "1 + 2 * 3 + 4 * 5 ..." -> "3 * 7 * 5 ..."
  while expr.count('+') > 0
    expr.sub!(/\d+ \+ \d+/){ |e| eval(e) }
  end

  # split the expression (eg) ['3', '*', '7', '*', '5', ...]
  expr = expr.split(' ')

  # parse the numbers (and ignore the operators) (eg) nums = [3, 7, 5, ...]
  nums = (0...expr.length).step(2).map{ |n| expr[n].to_i }

  # now take the product of the numbers (because at this point it's all multiplication)
  val = nums.inject(:*)

  return val.to_s
end

# sum up the result of each expression in maths
puts maths.map { |expr|

  # evaluate all inner parentheticals before proceeding
  while expr.count('(') > 0
    expr.sub!(/\([^\(\)]+\)/){ |e| dothemaths(e[1...-1]) }
  end

  dothemaths(expr).to_i
}.sum
