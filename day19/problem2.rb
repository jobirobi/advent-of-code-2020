...
112: 26 26
36: 67 26 | 125 16
74: 16 10 | 26 38'.split(/\n/)

messages = "aaaaaababbaaababaaaabbbb
...
# out: one array of strings?
# (eg) combinator(["a", "b"], ["aa", "bb"]) -> ["aaa", "abb", "baa", "bbb"]
#      combinator("a", "b") -> ["ab"]
#      combinator(["aaa", "abb", "baa", "bbb"], ["aba", "aab", "bba", "bab"]) -> [(16 combinations)]
def combinator(a, b)
  a.product(b).map{ |c| c.join('') }
end

# current rule format = ["n: i( j)( | k( l))", ...]
# (egs) ["8: 42", "11: 42 31", "44: 26 | 16", "2: 12 16 | 41 26"]
# convert rules to hash format n=>[([)i(, j])(, [k(, l)])]
# (egs) 8=>[42], 11=>[42, 31], 44=>[[26], [16]], 2=>[[12, 16], [41, 26]]
rules = rules.map { |line|
  (k, v) = line.split(': ')
  v.gsub!(/"/, '')
  [k.to_i, (v == "a" or v == "b") ? [v] : v.split(/ \| /).map{ |n| n.split(' ').map{ |m| m.to_i } }]
}.sort.to_h.map { |k, v|
  [k, (v.length == 1) ? v.flatten : v]
}.to_h

# solved = the set of rules keys which are resolved to strings or sets of strings
solved = rules.select{ |k, v| (v == ["a"] or v == ["b"]) }.keys.to_set

# now to solve all the unsolved up to the elements of rules 8 and 11, which are 42 and 31
until solved.include? 42 and solved.include? 31

  # only solve those which have not already been solved and are solvable
  rules.select{ |k, v| (!(solved.include? k) and (v.flatten.to_set <= solved)) }.each{ |k, v|

    # v.flatten == v: there are no nested arrays
    # (egs) 8=>[42], 11=>[42, 31]
    if v.flatten == v

      # v.length == 1: nothing to do here but copypasta
      # (eg) 8=>[42] -> rules[42]
      if v.length == 1
        rules[k] = rules[v[0]]

      # v.length != 1: use combinator
      # (eg) 11=>[42, 31] -> rules[42] X rules[31]
      else
        rules[k] = combinator(rules[v[0]], rules[v[1]]).uniq
      end

    # v.flatten != v: there are nested arrays
    # (egs) 44=>[[26], [16]], 2=>[[12, 16], [41, 26]]
    else

      # v.flatten.length == 2: either rule applies
      # (eg) 44=>[[26], [16]] -> rules[26] + rules[16]
      if v.flatten.length == 2
        rules[k] = (rules[v[0][0]] + rules[v[1][0]]).uniq

      # v.flatten.length != 2: either combination of rules applies
      # (eg) 2=>[[12, 16], [41, 26]] -> rules[12] X rules[16] + rules[41] X rules[26]
      else
        rules[k] = (combinator(rules[v[0][0]], rules[v[0][1]]) + combinator(rules[v[1][0]], rules[v[1][1]])).uniq
      end
    end
...
# simplifying this problem greatly are the facts that rules 42 and 31 are mutually exclusive...
raise "ERROR" unless (rules[42] & rules[31]).length == 0
# ...and all their component strings are 8 chars long
raise "ERROR" unless (rules[42].all?{ |s| s.length == 8 } and rules[31].all?{ |s| s.length == 8 })

# rules[8] = [[42], [42, 8]]
#   either match rule 42 or 42 followed by 8 (recursion occurs) = one or more 42
# rules[11] = [[42, 31], [42, 11, 31]]
#   either match rule 42 followed by 31 or 42 followed by 11 (recursion occurs) followed by 31
#   = equal numbers of 42 and 31
# rules[0] = [8, 11]
#   one or more 42 followed by equal numbers of 42 and 31 = a number of 42 followed by a smaller number of 31
rule42 = rules[42].join('|')
rule31 = rules[31].join('|')
rule0 = /^(#{rule42})+(#{rule31})+$/
rule42 = /#{rule42}/

# compare the number of 8-char substrings that match rules 42 or 31
yep = 0
messages.each{ |line|
  if (line =~ rule0)
    chunks = line.scan(/.{8}/)
    n = chunks.map{ |chunk| rule42.match?(chunk) ? 1 : 0 }.sum
    yep += 1 if n > (chunks.length - n)
  end
}
puts yep
