...
# taken_neighbors looks at the 8 adjacent squares and counts how many are '#'
def taken_neighbors(row, col)
  [
    (row - 1 >= 0 and col - 1 >= 0 and $seats[row - 1][col - 1] == '#'),
    (row - 1 >= 0 and                  $seats[row - 1][col]     == '#'),
    (row - 1 >= 0 and col + 1 < $w and $seats[row - 1][col + 1] == '#'),
    (col - 1 >= 0 and                  $seats[row][col - 1]     == '#'),
    (col + 1 < $w and                  $seats[row][col + 1]     == '#'),
    (row + 1 < $l and col - 1 >= 0 and $seats[row + 1][col - 1] == '#'),
    (row + 1 < $l and                  $seats[row + 1][col]     == '#'),
    (row + 1 < $l and col + 1 < $w and $seats[row + 1][col + 1] == '#')
  ].count(true)
end

# just for the record: I hate this Marshal business. it was a big pain point during testing
new_seats = Marshal.load(Marshal.dump($seats))
while true
  change = false

  # progress over the entire grid of $seats and toggle new_seats as prescribed
  (0...$l).each { |row|
    (0...$w).each { |col|
      case $seats[row][col]
      when '.'
      ...
      # when a seat is empty, it will be taken if none of its neighbors are taken
      when 'L'
      ...
      # when a seat is taken, it will be abandoned if 4+ of its neighbors are taken
      when '#'
        if taken_neighbors(row, col) >= 4 then
          new_seats[row][col] = 'L'
          change = true
        end
      end
  ...
  # $seats catches up to the changes
