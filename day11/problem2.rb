$seats = "LLLLLL.LL.LL.LLLLLL.LLL.L.LLLLLLLLLLLLLLLL.LLLLLLLLLLL.LLLL.LLLLLLL.LLLLLLLL.LLLLLLLLLLLLLLLLLLLL
...
LLLLLL.LLLLLL.LLLLLLLLLLL.LLLLLLL.LLLLLLLLLLLLLLL.LLLLLLLLL.LLLLLLL.LLLLLLLL.LLLLLLLLLLLLL.LLLLLL".split

# you may notice that $l and $w are defined differently than in problem1 to make my life easier
$l = $seats.length - 1
$w = $seats[0].length - 1

# taken_neighbors uses loops to follow the 8 cardinal paths until it hits the end or a '#' or 'L'
def taken_neighbors(row, col)

  # up and left: both indices decreasing
  upleft = ([row, col].min > 0 and (1..[row, col].min).each{ |i|
    next if $seats[row - i][col - i] == '.'
    break $seats[row - i][col - i] == '#'
  })

  # straight up: row index decreasing
  up = (row > 0 and (1..row).each{ |i|
    next if $seats[row - i][col] == '.'
    break $seats[row - i][col] == '#'
  })

  # up and right: row index decreasing and column index increasing at equal rates
  upright = ([row, $w - col].min > 0 and (1..[row, $w - col].min).each{ |i|
    next if $seats[row - i][col + i] == '.'
    break $seats[row - i][col + i] == '#'
  })

  # straight left: column index decreasing
  left = (col > 0 and (1..col).each{ |i|
    next if $seats[row][col - i] == '.'
    break $seats[row][col - i] == '#'
  })

  # straight right: column index increasing
  right = ($w - col > 0 and (1..$w - col).each{ |i|
    next if $seats[row][col + i] == '.'
    break $seats[row][col + i] == '#'
  })

  # down and left: row index increasing and column index decreasing at equal rates
  downleft = ([$l - row, col].min > 0 and (1..[$l - row, col].min).each{ |i|
    next if $seats[row + i][col - i] == '.'
    break $seats[row + i][col - i] == '#'
  })

  # straight down: row index increasing
  down = ($l - row > 0 and (1..$l - row).each{ |i|
    next if $seats[row + i][col] == '.'
    break $seats[row + i][col] == '#'
  })

  # down and right: both indices increasing
  downright = ([$l - row, $w - col].min > 0 and (1..[$l - row, $w - col].min).each{ |i|
    next if $seats[row + i][col + i] == '.'
    break $seats[row + i][col + i] == '#'
  })

  return [upleft, up, upright, left, right, downleft, down, downright].count(true)
end
...
while true
  ...
  (0..$l).each { |row|
    (0..$w).each { |col|
      ...
      when 'L'
        if taken_neighbors(row, col) == 0 then
          new_seats[row][col] = '#'
          change = true
        end

      # now when a seat is taken, it will be abandoned if 5+ of its cardinal neighbors are taken
      when '#'
        if taken_neighbors(row, col) >= 5 then
          new_seats[row][col] = 'L'
          change = true
        end
      end
  ...
  break if ! change
  $seats = Marshal.load(Marshal.dump(new_seats))
end

puts new_seats.map{ |row| row.count('#') }.sum
