...
# first round of selections are those that directly contain shiny gold bags
selection = rules.select{ |line| line =~ /contain.+shiny gold/ }.map{ |line| line.split[0..1].join(" ") }
solution = selection.to_set

while true do
  new_selection = Set[]
  selection.each { |color|
    # collect the bags that directly contain the last set of colors to be added to the solution
    new_selection.merge(rules.select{ |line| line =~ /contain.+#{color}/ }.map{ |line| line.split[0..1].join(" ") })
  }
  if new_selection <= solution
    # this iteration found no new colors to be added to the solution
    break

  else
    # add this iteration to the solution
    solution.merge(new_selection)

    # repeat using the new set
    selection = new_selection.to_a
  end
end
