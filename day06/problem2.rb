...
forms.each { |form|
  ...
  # for each letter given by the first member of a family, determine whether all members have the letter
  people[0].split(//).each { |c|
    total += 1 if people.all? { |p| p.include? c }
  }
}
