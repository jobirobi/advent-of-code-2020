$rules = "muted tomato bags contain 1 bright brown bag, 1 dotted gold bag, 2 faded gray bags, 1 posh yellow bag.
...
# returns the sum total of bags found inside of, and including, #{number} X #{color} bags
#
# example:
# children_of_bags(3, "mirrored yellow")
#
# the rule for "mirrored yellow" bags:
# "mirrored yellow bags contain 4 muted orange bags, 4 shiny black bags, 1 shiny green bag."
#
# if the child bags contained no more bags within, then
# children_of_bags(3, "mirrored yellow") = 3 + 3 * ( 4 + 4 + 1 ) = 30
#
# if the child bags contained 1, 2, 3 more bags within, respectively, then
# children_of_bags(3, "mirrored yellow") = 3 + 3 * ( (4 + 4 * 1) + (4 + 4 * 2) + (1 + 1 * 3) ) = 75
def children_of_bags(number, color)

  # children = ["4 muted orange", "4 shiny black", "1 shiny green"]
  children = $rules.find{ |line| line =~ /^#{color}/ }.scan(/\d+ \w+ \w+/)
  return number if children.length == 0

  return number + number * children.map{ |child|
    # child = ["4", "muted", "orange"]
    child = child.split
    ...
    # how many bags are inside the 4 muted orange bags?
    children_of_bags(num, clr)
  }.sum

end

puts children_of_bags(1, "shiny gold") - 1
